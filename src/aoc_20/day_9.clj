...
(defn sum-pairs [numbers]
  (let [size (count numbers)]
    (->> (for [x (range size)
               y (range size)
               :let [sum (when-not (= x y) (+ (nth numbers x)
                                              (nth numbers y)))]]
           sum)
         (remove nil?)
         (set))))

(defn part-1 [input preamble]
  (let [all-numbers (into []
                          (for [number input]
                            (Long/parseLong number)))]
    (loop [remaining (range preamble (count all-numbers))]
      (let [idx (first remaining)
            current (nth all-numbers idx)
            numbers (subvec all-numbers (- idx preamble) idx)
            sums (sum-pairs numbers)]
        (if (sums current)
          (recur (rest remaining))
          current)))))

; (part-1 (read-lines "day9-example-input.txt") 5)
; (part-1 (read-lines "day9-input.txt") 25)
