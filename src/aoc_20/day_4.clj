...
(defn parse-passport [passport]
  (let [pairs (s/split passport #" ")]
    (into {} (map split-pair pairs))))

(def required-keys #{:byr :iyr :eyr :hgt :hcl :ecl :pid})

(defn required-keys? [all-keys]
  (empty? (difference required-keys (set all-keys))))

(defn valid-passport? [passport]
  (-> (newline->space passport)
      (parse-passport)
      (keys)
      (required-keys?)))
