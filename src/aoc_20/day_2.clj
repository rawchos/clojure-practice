...
(defn letter-count [string letter]
  (let [letter-map (->> (seq string)
                        (map str)
                        (frequencies))]
    (get letter-map letter 0)))

(defn valid-password? [password letter minimum maximum]
  (let [occurences (letter-count password letter)]
    (and (>= occurences minimum)
         (<= occurences maximum))))
...
(defn part-1 [input]
  (->> (for [line input
             :let [[min max letter _ password] (-> (s/replace line #"[-:]" " ")
                                                   (s/split #" "))]]
         (if (valid-password? password letter (to-int min) (to-int max))
           1
           0))
       (reduce +))) 
...
(defn char-at 
  "Returns the character found in the position specified. Assumes no zero
   index."
  [string position]
  (if (or (> position (count string))
          (= 0 position))
    nil
    (str (nth string (dec position)))))

(defn valid-positions? [password letter p1 p2]
  (let [first-char  (char-at password p1)
        second-char (char-at password p2)]
    (cond
      (= first-char second-char letter) false
      (= first-char letter) true
      (= second-char letter) true
      :else false)))

(defn part-2 [input]
  (->> (for [line input
             :let [[p1 p2 letter _ password] (-> (s/replace line #"[-:]" " ")
                                                 (s/split #" "))]]
         (when (valid-positions? password letter (to-int p1) (to-int p2))
           true))
       (remove nil?)
       (count)))
