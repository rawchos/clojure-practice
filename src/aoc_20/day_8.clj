...
;; 0 to 1 (acc +1) and jmp +4 sets the next instruction to the other
;; acc +1 near the bottom. After it increases the accumulator from 1 to 2,
;; jmp -4 executes, setting the next instruction to the only acc +3. It
;; sets the accumulator to 5, and jmp -3 causes the program to continue
...
(defn process-instruction [{:keys [instruction-line
                                   accumulator
                                   processed-instructions] :as state}
                           instruction]
  (let [[_ action string-f amount] (re-find #"^(\w+) (\+|-)(\d+)$" instruction)
        f (resolve (symbol string-f))
        int-amount (Integer/parseInt amount)]
    (assoc 
     (case (keyword action)
       :jmp (assoc state
                   :instruction-line (f instruction-line int-amount))
       :acc (assoc state
                   :instruction-line (inc instruction-line)
                   :accumulator (f accumulator int-amount))
       :nop (assoc state
                   :instruction-line (inc instruction-line))
       state)
     :processed-instructions (conj processed-instructions instruction-line))))
...
(defn part-1 [instructions]
  (loop [{:keys [instruction-line
                 ...
                 processed-instructions] :as state} {:instruction-line 0
                                                     ...
                                                     :accumulator 0}]
    (if (in? processed-instructions instruction-line)
      accumulator
      (recur (process-instruction state (nth instructions instruction-line))))))

; (part-1 (read-lines "day8-example-input.txt"))
; (part-1 (read-lines "day8-input.txt"))
