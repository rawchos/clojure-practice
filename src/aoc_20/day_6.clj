...
(defn modified-count-batch [batch]
  (let [group-count (-> (s/split batch #"\n")
                        (count))
        answers (-> (s/replace batch #"\n" "")
                    (s/split #"")
                    (frequencies))]
    (->> (for [[_ num] answers]
           (if (= num group-count) 1 0))
         (reduce +))))

(defn part-2 [batches]
  (->> (s/split batches #"\n\n")
       (map modified-count-batch)
       (reduce +)))

; (part-2 (read-input "day6-input.txt"))